...
class Growatt2MQTT:
    ...
    def __init__(self):
        ...
        self.__loops_since_last_settings_read = 0
        return None
        ...
        while True:
            ...
            for inverter in inverters:
                ...
                try:
                    ...
                    if info is None:
                        continue
                    if self.__loops_since_last_settings_read > 600:  # every 10 minutes
                        settings_data = growatt.read_settings()
                        if settings_data:
                            # Senden an MQTT (evtl. separates Topic oder Retain Flag setzen)
                            # publish_mqtt(settings_data, topic + "/settings", retain=True)
                            self.__mqtt_client.publish(
                                self.__mqtt_topic + "/settings", json.dumps(settings_data), 0, properties=self.__properties, retain=True)
                            pass
                        self.__loops_since_last_settings_read = 0
                    else:
                        self.__loops_since_last_settings_read += 1
